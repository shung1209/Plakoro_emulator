...
const COMPACT_PORTRAIT_SIZE: int = 136
...
static func present(
    ...
    if StringName(resolved["mode"]) == &"image_2d":
        if _present_image(
            target,
            String(resolved["image_path"]),
            display_name,
            portrait_size
        ):
            return resolved

    _present_placeholder(
        target,
        display_name,
        pokemon_id,
        portrait_size
...
static func _present_placeholder(
    target: Control,
    display_name: String,
    pokemon_id: StringName,
    portrait_size: int
